...
-- Historical fallback time is set to 12:00:00 UTC for daily-only legacy records.
...
SELECT
  STR_TO_DATE(CONCAT(wd.date, ' 12:00:00'), '%Y-%m-%d %H:%i:%s') AS measured_at,
  wd.sea_temperature,
  'legacy_backfill' AS source,
  'Backfilled from weather_daily.sea_temperature (default 12:00:00 UTC)' AS note,
