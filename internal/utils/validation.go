...
// EnvPathOrDefault returns env path when safe, otherwise a default fallback path.
func EnvPathOrDefault(envKey, defaultPath string) string {
	value := os.Getenv(envKey)
	if value == "" {
		return defaultPath
	}
	if !IsSafePath(value) {
		log.Printf("Warning: %s path '%s' is unsafe, falling back to default", envKey, value)
		return defaultPath
	}
	return value
}
